...
The frontend generates and persists a throwaway private key in the browser's localStorage on first "Connect Wallet" — in this deployment that generated address is the freelancer account listed above, so submit_work and resolve can be called directly from the frontend.

fund_escrow is client-only and was executed from the client account via GenLayer Studio, since that account isn't the one persisted in this browser session — both are real, independently controlled authorized participant accounts, matching the configuration documented above.
